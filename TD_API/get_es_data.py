...
# make a request
    ...
    # convert it dictionary object
        ...
        print ("/ES bid = "+ str((data['/ES']['bidPriceInDouble'])))
        print ("/ES bid size = " + str((data['/ES']['bidSizeInLong'])))
        print ("/ES ask = "+ str((data['/ES']['askPriceInDouble'])))
        print ("/ES ask size = " + str((data['/ES']['askSizeInLong'])))
        print("/ES last price = " + str((data['/ES']['lastPriceInDouble'])))
        print("/ES last size = " + str((data['/ES']['lastSizeInLong'])))
        quote_time =(data['/ES']['quoteTimeInLong'])/1000
        quote_time = datetime.fromtimestamp(quote_time).strftime('%Y-%m-%d %H:%M:%S.%f')
        print (quote_time)
        trade_time =(data['/ES']['tradeTimeInLong'])/1000
